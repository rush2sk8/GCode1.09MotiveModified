...
//        mw.setSize(java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds().width, java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds().width);
            ...
            public void actionPerformed(final java.awt.event.ActionEvent evt) {
               SwingUtilities.invokeLater(new Runnable(){
					...
					public void run(){
						...
						new GetPointsFromScreen().getPoints(p);
						System.out.println(p[0]+":"+p[1]);

						//press the start button and execute sending grbl commands
						try{
							Robot bot = new Robot();
							bot.mouseMove((int)p[0].getX(), (int)p[0].getY());
							Thread.sleep(100);
						...
						System.out.println("after send started");
    ...
    private void sendButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sendButtonActionPerformed
        ...
        System.out.print("done with sending commands?");
    ...
    public void fileStreamComplete(String filename, boolean success) {
		...
		try{
			Robot bot = new Robot();
			bot.mouseMove((int)p[1].getX(), (int)p[1].getY());
			Thread.sleep(100);
			bot.mousePress(InputEvent.BUTTON1_MASK);
			bot.mouseRelease(InputEvent.BUTTON1_MASK);
		}catch(AWTException | InterruptedException e){
	...
	protected  class GetPointsFromScreen {
		...
		public Point[] getPoints(){
			...
			RunnableFuture<Point[]> r = new RunnableFuture<Point[]>() {
				...
				public void run() {
					...
					final JLabel pointLabel = new JLabel(
							"Click on the start and stop button");
					...
					screenLabel.addMouseListener(new MouseAdapter() {
						...
						public void mouseClicked(MouseEvent me) {
							...
							if(numClicks == 1) {
								...
								p[1] = me.getPoint();
								pointLabel.setText("Stop button registered \n Click the Done button!");
								...
								p[0] = me.getPoint();
								pointLabel.setText("Start button registered!");
								numClicks++;
							}else {
								System.err.println("not right number of clicks");
								System.exit(-1);
							}
		...
		public void getPoints(final Point[] p){
			...
			final JLabel screenLabel = new JLabel(new ImageIcon(screen));
			JScrollPane screenScroll = new JScrollPane(screenLabel);
			screenScroll.setPreferredSize(new Dimension((int)(screenSize.getWidth()/1.2),(int)(screenSize.getHeight()/1.2)));
			screenScroll.getVerticalScrollBar().setUnitIncrement(20);
			JPanel panel = new JPanel(new BorderLayout());
			panel.add(screenScroll, BorderLayout.CENTER);
